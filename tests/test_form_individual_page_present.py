...
def test_form_present_in_first_job_for_each_page():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False,args=["--start-maximized"])
        context = browser.new_context()
        page = context.new_page()

        url = "https://jobs.magnumlog.com/"
        ...
        if not page.locator("#job-listings-wrapper").is_visible():
            ...
            try:
                page.get_by_text("No jobs match your search criteria. Please try a different search.").wait_for(
                    timeout=50000)
                print(f"No jobs are visible in the {url}.\n")
            except:
                ...
                page.locator(
                    "#job-listings-wrapper .listing h3.listing-title").first.click(), f"Clicking on the first job was not successful for {url}.\n"
                print(f"{url}: {job_clicked}")
            try:
                expect(page.get_by_text("Start Here", exact=True)).to_be_visible(
                    timeout=50000), f"Start Here is not visible for {url}.\n"
            except:
            ...
            try:
                ...
                expect(page.locator("label[for='clickwrap-consent-checkbox']")).to_contain_text(
                    american_consent_text), f"Consent text was not visible for {url}.\n"
                ...
                expect(page.get_by_role("list").filter(has_text="Please keep me up to date")).to_contain_text(
                    canadian_consent_text)
